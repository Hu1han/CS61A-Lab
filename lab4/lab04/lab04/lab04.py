...
def double_eights(n):
    ...
    def bi(a,b):
        if (a == b and a == 8)or a==True:
            return True
        else :
            return b
    if reduce(bi,result) == True:
        return reduce(bi,result)
    else:
        return False
...
def summation(n, term):
    ...
    "*** YOUR CODE HERE ***"
    from functools import reduce
    return reduce(lambda x,y: x+y,list(map(term,range(1,n+1))))
...
def count_palindromes(L):
    ...
    2
    """
    return len(list(filter(lambda s: (s.lower())[::-1]== (s.lower()),L)))
